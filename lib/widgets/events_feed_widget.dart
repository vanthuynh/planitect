...
List<AustinYogaWork> austin = [
  ...
  Widget EventsFeed() {
    ...
    // constantly listen to events from all users
...
List eventSavedByUsers = [];
 ...
 FirebaseFirestore.instance.collection('events').doc(eventData!.id).set({
  ...
  EventItem(DocumentSnapshot event) {
    ...
    // Get all data of users that associated with current events shown on home_screen
    DataController dataController = Get.find<DataController>();
    // find the first user that has this event --> the one that created this event
